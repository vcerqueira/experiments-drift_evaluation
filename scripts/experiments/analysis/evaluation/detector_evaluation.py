...
p.save(f"{OUTPUT_DIR}/plot1_hypertuning.pdf", width=10, height=6)
...
latex_table_abr = r_ht_abp_idx_tab.T.to_latex(
    ...
    caption='Performance metrics of drift detectors across different datasets abrupt',
...
latex_table_grd = r_ht_grd_idx_tab.T.to_latex(
    ...
    multicolumn=True,
    multicolumn_format='c',
    caption='Performance metrics of drift detectors across different datasets gradual',
    label='tab:combined_metrics',
    position='htbp'
)
...
p2.save(f"{OUTPUT_DIR}/plot2_real_abrupt_barplots.pdf", width=12, height=6)
...
p3.save(f"{OUTPUT_DIR}/plot3_real_gradual_barplots.pdf", width=12, height=6)
...
p5.save(f"{OUTPUT_DIR}/plot5_f1_vs_far.pdf", width=12, height=8)
...
p6 = (
        ...
        + p9.theme(legend_position='top',
                   strip_background=p9.element_text(color='lightgrey'))
        + p9.labs(title='', x='log MDT', y='F1')
)

p6.save(f"{OUTPUT_DIR}/plot6_f1_vs_mdt.pdf", width=12, height=8)
